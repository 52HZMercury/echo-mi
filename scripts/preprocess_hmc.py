...
def preprocess_hmc_dataset(source_dir, dest_dir, metadata_path):
    """
    预处理HMC数据集，从AVI视频中提取帧并保存为.pt文件。
    """
    ...
    for _, row in tqdm(metadata.iterrows(), total=len(metadata), desc="Processing HMC-QU"):
        patient_id = row['name']
        ...
        if a2c_video_path.exists() and not a2c_pt_path.exists():
            try:
                start, end = int(row["RF_A2C"]), int(row["EC_A2C"])
        ...
        if a4c_video_path.exists() and not a4c_pt_path.exists():
            try:
                start, end = int(row["RF_A4C"]), int(row["EC_A4C"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Preprocess HMC-QU dataset.")
    parser.add_argument('--source_dir', type=str, required=True, help='Directory containing the raw AVI videos (data_ori_changename).')
    parser.add_argument('--dest_dir', type=str, required=True, help='Directory to save the processed .pt files.')
    parser.add_argument('--metadata_path', type=str, default='data/160version.csv', help='Path to the metadata CSV file.')

    args = parser.parse_args()

    preprocess_hmc_dataset(args.source_dir, args.dest_dir, args.metadata_path)
