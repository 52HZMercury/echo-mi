...
from .components.k1_modules_3d import VSSBlock
    ...
    def __init__(
        ...
        if self.channel_token:
            ...
            self.mamba = VSSBlock(
                ...
                drop_path=0.1,
                use_conv=use_conv,
                use_depth=use_depth,  # 关键：这里如果为 False，就不会初始化报错的 depth_fc
                mode=depth_mode,
...
class ResidualMambaEncoder(nn.Module):
    def __init__(
        ...
        # --- FIX: 重置这些参数以适应不同输入尺寸 ---
        # 原始硬编码参数会导致不同分辨率下的 VSSBlock 初始化错误
        use_convs = [True, True, True, False, False]

        # 强制关闭 use_depths，避免因特征图尺寸变化导致的维度不匹配
        use_depths = [False] * n_stages

        # 将相关参数设为默认值
        d_depth_strides = [0] * n_stages
        d_depth_squeezes = [0] * n_stages
        d_depth_outs = [0] * n_stages
        # ------------------------------------------
    ...
    def forward(self, x):
        if self.stem is not None:
            x = self.stem(x)
        stem = x
        # 注意: 这里的 max_pool 可能会导致 Z 轴过早压缩，
        # 如果输入 Z=32，pool 后为 16。结合下采样，层深处可能变为 1 或 2。
        # 建议检查这里是否需要根据输入尺寸动态调整，但一般 max_pool 2x2x2 是安全的。
...
class UMambaEnc(nn.Module):
    def __init__(
        ...
        # 2. 新增：分类分支 (Classification Head)
        # 提取编码器最后一层的输出通道数
        bottleneck_channels = features_per_stage[-1]

        self.avgpool = nn.AdaptiveAvgPool3d(1)  # 全局平均池化
        self.classification_head = nn.Sequential(
            nn.Flatten(),
            nn.Linear(bottleneck_channels, bottleneck_channels // 2),
            nonlin(**nonlin_kwargs),
            nn.Dropout(p=0.1),
            nn.Linear(bottleneck_channels // 2, 1)  # 输出 2 分类 logits
        )

    def forward(self, x_a2c: torch.Tensor, x_a4c: torch.Tensor, return_features: bool = False):
        skips, stem = self.encoder(x_a2c)

        # --- 分类分支 ---
        # 提取 Bottleneck 特征 (skips 的最后一个元素)
        bottleneck_feat = skips[-1]
        fused_feat = self.avgpool(bottleneck_feat)  # (B, C, 1, 1, 1)
        fused_feat = torch.flatten(fused_feat, 1)  # (B, C)
...
# UNetResDecoder 和 UMambaEnc 保持不变 (略过以节省篇幅，请保留你原有的代码)
# ... [UNetResDecoder class code] ...
# ... [UMambaEnc class code] ...
...
def get_umamba_enc_dc_k1_3d_from_plans(
    ...
    # --- 针对 32 帧输入的下采样策略调整 ---
    # Layer 0 (Stem): Stride [1,1,1] -> Out: 32 (After stem pool: 16)
    # Layer 1: Stride [1,2,2] -> Z保持
    # Layer 2: Stride [2,2,2]
    # ...
    ...
    kwargs = {
        "UMambaEnc": {
            # 这里设置正确的 Input Size 非常重要，它影响内部 Channel Token 的判断逻辑
    ...
    model = network_class(
        ...
        n_stages=num_stages,
        features_per_stage=[min(32 * 2 ** i, 320) for i in range(num_stages)],
        conv_op=conv_op,
