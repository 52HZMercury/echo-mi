...
class MIMamba(nn.Module):
    ...
    def forward(self, a2c_video, a4c_video, return_features=False):
        ...
        outs_a2c = self.vit(a2c_video)
        outs_a4c = self.vit(a4c_video)


        # 只使用最后一个特征层（最深层的语义信息）
        last_feature_a2c = outs_a2c[3]
        last_feature_a4c = outs_a4c[3]

        # 通过encoder5转换通道数
        a2c_enc_hidden = self.encoder5(last_feature_a2c) # Size([16, 768, 2, 14, 14])
        a4c_enc_hidden = self.encoder5(last_feature_a4c) # Size([16, 768, 2, 14, 14])

        # # 特征融合
        # stacked_features = torch.stack([a2c_enc_hidden, a4c_enc_hidden], dim=0)
...
if __name__ == "__main__":
    ...
    # 测试输入
    x = torch.randn(16, 3, 32, 224, 224)  # batch_size=2, 1通道, 64x64x64体积
    x = x.cuda()  # 或者 x.to('cuda')
    output = model(x)
    print(f"Output shape: {output.shape}")  # 应该是 torch.Size([2, 2])
