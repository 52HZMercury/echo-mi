...
class LKMClassification(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
        ...
        # --- 新增：修复 Hydra ListConfig 类型不兼容问题 ---
        # 将 OmegaConf 的 ListConfig 转换为标准的 Python list
        if isinstance(kernel_sizes, (ListConfig, list, tuple)):
            if isinstance(kernel_sizes, ListConfig):
                kernel_sizes = OmegaConf.to_container(kernel_sizes, resolve=True)

        if isinstance(strides, (ListConfig, list, tuple)):
            if isinstance(strides, ListConfig):
                strides = OmegaConf.to_container(strides, resolve=True)

        if isinstance(features_per_stage, (ListConfig, list, tuple)):
            if isinstance(features_per_stage, ListConfig):
                features_per_stage = OmegaConf.to_container(features_per_stage, resolve=True)

        if isinstance(n_conv_per_stage, (ListConfig, list, tuple)):
            if isinstance(n_conv_per_stage, ListConfig):
                n_conv_per_stage = OmegaConf.to_container(n_conv_per_stage, resolve=True)
        # ----------------------------------------------------
        ...
        # 定义分类头
        last_stage_channels = features_per_stage[-1]
        ...
        self.fc = nn.Linear(last_stage_channels, num_classes)

    def forward(self, x_a2c: torch.Tensor, x_a4c: torch.Tensor,return_features: bool = False):
        skips = self.encoder(x_a4c)
        last_feature_map = skips[-1]
        x = self.avg_pool(last_feature_map)
        x = x.view(x.size(0), -1)
        x = self.dropout(x)
        logits = self.fc(x)
        # 返回分类输出
...
class LKMUNet(nn.Module):
    def __init__(self,
        ...
        self.encoder = ResidualBiMambaEncoder(input_channels, n_stages, features_per_stage, conv_op, kernel_sizes,
                                              strides,
...
def main():
    ...
    # 1. Configuration
    # 注意: PyTorch Conv3d 的输入格式通常为 (Batch, Channels, Depth, Height, Width)
    # 如果你的 D=16, H=128, W=128，建议检查维度顺序是否正确
    INPUT_SIZE = (1, 3, 16, 128, 128)
    ...
    # 6. Calculate FLOPs (关键修改部分)
    print("-" * 30)
    print("Calculating FLOPs...")
    try:
        # -----------------------------------------------------------
        # 修改点：复制一个模型专门用于计算 FLOPs
        # 这样 thop 的 hooks 只会挂载在副本上，不会污染原模型
        # -----------------------------------------------------------
        model_for_flops = copy.deepcopy(model)

        # thop 无法自动识别 Mamba 层的计算量，这里计算的主要是卷积和线性的量
        # 结果可能会偏小，这是正常现象
    ...
    # Warmup
    # 此时 model 没有任何 thop 的钩子，运行不会报错，且速度真实
    with torch.no_grad():
